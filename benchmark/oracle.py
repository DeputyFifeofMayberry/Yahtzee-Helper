...
from .models import DecisionStateSnapshot, PolicyDecision, RolloutReferenceComparisonRecord
...
@dataclass
class RolloutReferencePolicy:
    name: str = "rollout_reference"
    ...
    def decide(self, state: GameState, advisor: YahtzeeAdvisor) -> PolicyDecision:
        ...
        if not candidates:
            raise ValueError("No candidate decisions available for rollout reference.")
        evaluation_seeds = self.evaluation_seeds(state, self.rollouts_per_action)

        best_decision: PolicyDecision | None = None
        best_value = float("-inf")
        for candidate in candidates:
            value = self.estimate_action_value(state, candidate, advisor, evaluation_seeds)
            if value > best_value:
                best_value = value
                best_decision = candidate

        if best_decision is None:
            raise ValueError("Rollout reference failed to choose an action.")
    ...
    def evaluation_seeds(self, state: GameState, count: int) -> list[int]:
        base = _stable_seed_from_state(state)
...
def compare_policies_to_rollout_reference(
    snapshots: list[DecisionStateSnapshot],
    policies: list[Policy],
    reference: RolloutReferencePolicy,
    advisor: YahtzeeAdvisor | None = None,
    evaluation_rollouts: int | None = None,
    on_progress: Callable[[int, int], None] | None = None,
) -> list[RolloutReferenceComparisonRecord]:
    advisor = advisor or YahtzeeAdvisor()
    records: list[RolloutReferenceComparisonRecord] = []
    decision_cache: dict[DecisionKey, float] = {}
    state_decision_cache: dict[tuple[tuple[tuple[int | None, ...], int], tuple[int, ...], int, int], PolicyDecision] = {}
    ...
    for idx, snapshot in enumerate(snapshots, start=1):
        ...
        reference_decision = state_decision_cache.get(state_key)
        if reference_decision is None:
            reference_decision = reference.decide(clone_state(state), advisor)
            state_decision_cache[state_key] = reference_decision

        rollout_count = evaluation_rollouts if evaluation_rollouts is not None else max(16, reference.rollouts_per_action // 2)
        evaluation_seeds = reference.evaluation_seeds(state, rollout_count)
        reference_value = reference.estimate_action_value(
            state,
            reference_decision,
            advisor,
            evaluation_seeds,
        ...
        for policy in policies:
            ...
            matched = _decisions_match(decision, reference_decision)
            if matched:
                policy_value = reference_value
            else:
                policy_value = reference.estimate_action_value(
                    state,
                    decision,
                    advisor,
                    evaluation_seeds,
            ...
            records.append(
                RolloutReferenceComparisonRecord(
                    ...
                    provenance_seed=snapshot.provenance_seed,
                    provenance_game_id=snapshot.provenance_game_id,
                    provenance_policy=snapshot.provenance_policy,
                    dice=snapshot.dice,
                    turn_index=snapshot.turn_index,
                    roll_number=snapshot.roll_number,
                    score_signature=_serialize_score_signature(snapshot.score_signature),
                    policy_action=_serialize_decision(decision),
                    reference_action=_serialize_decision(reference_decision),
                    matched_rollout_reference=matched,
                    estimated_policy_value=round(policy_value, 6),
                    estimated_reference_value=round(reference_value, 6),
                    estimated_regret_vs_reference=round(max(0.0, reference_value - policy_value), 6),
                    evaluation_rollouts=len(evaluation_seeds),
                    tags=snapshot.tags,
                )
            )
...
def _serialize_decision(decision: PolicyDecision) -> str:
    if decision.is_score:
        return f"score:{decision.category.value if decision.category else 'unknown'}"
    held = ','.join(str(v) for v in (decision.held_dice or ()))
    return f"hold:{held}"


def _serialize_score_signature(signature: tuple[tuple[int | None, ...], int]) -> str:
    return json.dumps({"scores": list(signature[0]), "yahtzee_bonus": signature[1]}, sort_keys=True, separators=(",", ":"))


def _stable_seed_from_state(state: GameState) -> int:
    payload = {
        "dice": list(state.current_dice),
        "roll_number": state.roll_number,
        "turn_index": state.turn_index,
        "score_signature": list(state.scorecard.score_signature()[0]),
        "yahtzee_bonus": state.scorecard.score_signature()[1],
    }
