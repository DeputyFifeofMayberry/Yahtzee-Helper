...
def test_heavy_settings_trigger_auto_downgrade_in_browser_safe_mode():
    settings = BenchmarkSettings(full_games=300, state_sample_games=80, state_sample_size=300, rollout_reference_rollouts=60)
    plan = plan_benchmark_run(settings=settings, mode="balanced", include_move_quality=True)
    assert plan.auto_downgraded_settings
    assert plan.settings.full_games <= 80
    assert plan.settings.state_sample_size <= 100
...
def test_move_quality_disabled_resets_reference_work():
    settings = BenchmarkSettings(state_sample_games=9, state_sample_size=12, rollout_reference_rollouts=8)
